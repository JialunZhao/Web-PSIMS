...
//		}
	...
	public void getCustomeName(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String customeName = request.getParameter("q");
//		String customeName = request.getParameter("term");
		List<TbCustomer> cuList=new ArrayList<TbCustomer>();
		TbCustomerExample example=new TbCustomerExample();
		com.ai.psims.web.model.TbCustomerExample.Criteria criteria=example.createCriteria();
		criteria.andCustomerNameLike("%"+customeName+"%");
		criteria.andEndtimeIsNull();
		cuList=customerBusiness.customerQuery(example);
		JSONObject data = new JSONObject();
		data.put("customerList", JSON.toJSONString(cuList));
		responseSuccess(response, "SUCCESS", data);
	}
